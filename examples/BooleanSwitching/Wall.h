...
// to compare charts, we do not check the id number
	bool operator==(const Wall &other) const {
		const std::vector < double > & lbounds1 = rect_.lower_bounds;
		const std::vector < double > & ubounds1 = rect_.upper_bounds;
		const std::vector < double > & lbounds2 = other.rect_.lower_bounds;
		const std::vector < double > & ubounds2 = other.rect_.upper_bounds;
...
Wall::isFixedPoint ( void ) const {
	bool output = true;
	const std::vector < double > & lbounds = rect_ . lower_bounds;
	const std::vector < double > & ubounds = rect_ . upper_bounds;
