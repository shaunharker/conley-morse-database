...
// Just a hack to update the parameter sigma, will be removed later
void update_parameters ( BooleanBox & box, const RectGeo & param );
void update_parameters ( std::vector < BooleanBox > & box, const RectGeo & param );
...
void update_parameters ( std::vector < BooleanBox > & box, const RectGeo & param ) {
	for ( unsigned int i=0; i<box.size(); ++i ) {
		update_parameters ( box[i], param );
	}
}
// Given a parameter, we update the booleanbox
void update_parameters ( BooleanBox & box, const RectGeo & param ) { 

	std::vector < double > lbsigma = box . sigma . lower_bounds;
	std::vector < double > ubsigma = box . sigma . upper_bounds;

	std::vector < double > centerbox;
	for ( unsigned int i=0; i<box.rect.dimension(); ++i ) {
		centerbox . push_back ( 0.5*(box.rect.lower_bounds[i]+box.rect.upper_bounds[i]) );
	}

// @@@@ --- WARNING : THRESHOLD HARDCODED --- @@@@
	if ( centerbox [ 0 ] > 8.0 ) {
		lbsigma [ 0 ] = lbsigma [ 0 ] + param . lower_bounds [ 0 ];
		ubsigma [ 0 ] = ubsigma [ 0 ] + param . upper_bounds [ 0 ];
	}

	box . sigma = chomp::Rect ( box.sigma.dimension(), lbsigma, ubsigma ); 
}
